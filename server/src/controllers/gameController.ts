...
import { Team, type TeamDocument } from '../models/Team.js';
...
const ensureProgressEntry = (team: TeamDocument, roundNumber: number) => {
...
const findRoundOrThrow = async (roundNumber: number) => {
...
const sanitizeTeam = (team: TeamDocument) => ({
  id: team._id,
  name: team.name,
  status: team.status,
...
const computeTotalTime = (team: TeamDocument) => {
